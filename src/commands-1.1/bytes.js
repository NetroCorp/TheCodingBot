...
exports.run = async(client, message, footertxt, prefix, command) => {
		...
		if (maxnum >= counter){
			if(command=="write"){
				...
				if(collectable){ 
					...
					if(!row){ErrorHandler("You do not have a account yet. Run ``/tcg bytes-create`` to make one."); }
				...
				message.channel.send({embed:{title:"TCG Bot | Bytes | Collect",color:colors[2],description:"You need "+num+" Bytes for your account file. Run ``/tcg write`` to write them to your account file.",footer:{text:footertxt} }})
	...
	if(command == "bytes-award"){
		if(message.author.id == TCG){
			...
			con.query('SELECT * FROM bytesdata WHERE id = "'+personid+'"', (err,row) => {
				...
				} else { ErrorHandler("This user does not have a account yet. Run ``/tcg bytes-create`` to make one."); return; }
			});
	...
	if(command == "bytes-bal"){
		...
		if(!balargs[2] || balargs[2] == null || balargs[2] == "" || balargs[2] == " "){
			...
			var personid = person.replace(/[<@!>]/g, '');
			con.query('SELECT * FROM bytesdata WHERE id = "'+personid+'"', (err,row) => {
				if(!row){ ErrorHandler("This user does not have a account yet. Run ``/tcg bytes-create`` to make one."); return; }
