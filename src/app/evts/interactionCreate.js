...
module.exports = {
	...
	execute: async(app, interaction) => {
		...
		if (interaction.isChatInputCommand()) {
			...
			interaction.serverInfo = await app.DBs.TheCodingBot.serverSettings.findOne({ where: { serverID: interaction.guild.id } });
			if (interaction.serverInfo != null) interaction.serverInfo.logging = await app.DBs.TheCodingBot.logging.findOne({ where: { serverID: interaction.guild.id } });
