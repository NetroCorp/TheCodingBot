	...
	loadEvents = (events) => {
		...
		for (let i = 0; i < events.length; i++) {
			...
			try {
				...
				if (rqEvent.type == "rest") this.app.client.rest.on(rqEvent.name, rqEvent.execute.bind(null, this.app));
				else this.app.client.on(rqEvent.name, rqEvent.execute.bind(null, this.app));
				this.app.logger.info("SYS", `Loaded ${(rqEvent.type == "rest") ? "REST" : ""} event: ${eventName} by ${rqEvent.author.join(", ")} in ${new Date().getTime() - startTime}ms.`);
			} catch (Ex) {
